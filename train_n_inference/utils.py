...
from transformers import LlamaForCausalLM, Trainer, DataCollatorForLanguageModeling
...
class DataCollatorForLanguageModeling_multi(DataCollatorForLanguageModeling):
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        batch = super().torch_call(examples)
        #batch["task_mask"] = torch.tensor([example["task_mask"] for example in examples])
        return batch

class LlamaForCausalLM_multi(LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.lm_head_1 = self.lm_head
        self.lm_head_2 = deepcopy(self.lm_head)
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            if task_name is not None:
                if task_name == "Prolog":
                    logits = self.lm_head_1(hidden_states)
                elif task_name == "CoT":
                    logits = self.lm_head_2(hidden_states)
                else:
                    raise ValueError("flag value incorrect!")
            elif task_mask is not None:
                logits = task_mask.unsqueeze(2) * self.lm_head_1(hidden_states) + (1-task_mask.unsqueeze(2)) * self.lm_head_2(hidden_states)
            else:
                raise Exception("Don't know which head to use.")
        logits = logits.float()
...
class NLPDataCollator:
    ...
    def collate_batch(self, features: List[Union[InputDataClass, Dict]]) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(first, dict):
            ...
            raise Exception("It seems the input for data collator is not a dictionary!")
...
class SteamGenerationMixin(PeftModelForCausalLM, GenerationMixin):
    ...
    def from_pretrained(cls, model, model_id, **kwargs):
        ...
        # Xiaocheng:
        # The line above seems problematic. Function set_peft_model_state_dict has no return value. Use the line below instead.
        ...
        if getattr(model, "hf_device_map", None) is not None:
            ...
            # Xiaocheng:
            # The line above is problematic. The model inherits from PeftModel, of which .peft_config is a dict. Use .peft_type instead.
